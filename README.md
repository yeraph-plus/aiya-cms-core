...
cd front-station
npm install
cp .env.example .env        # AIYA_SITE_URL / AIYA_WP_API_URL / AIYA_API_TIMEOUT_MS / AIYA_ALLOW_LOCAL_HTTP
...
# → copy the output to front-station/src/lib/aiya/contracts.snapshot.json
...
`wp-content/themes/`) and mirror to the other — they must stay
byte-identical.

 ...
 * holder through the shared user typeahead. The users list gains a
 * membership-status column linking into this view — the money-fact
 * counterpart of the credit ledger. Reuses the credit ledger's user
 * picker AJAX so both screens behave identically.
 */
final class PaymentsAuditPage implements Module
{
        ...
        <div class="wrap">
            ...
            <form method="get" class="aiya-core-filters" style="margin:12px 0;">
                ...
                <select name="source">
                    ...
                    <option value="epay" <?php selected($source, 'epay'); ?>><?php esc_html_e('Epay', 'aiya-core'); ?></option>
                    <option value="afdian" <?php selected($source, 'afdian'); ?>><?php esc_html_e('Afdian', 'aiya-core'); ?></option>
                </select>
            ...
            <table class="widefat striped">
                ...
                <tbody>
                <?php if ($result['items'] === []) : ?>
                    <tr><td colspan="6"><?php esc_html_e('No payments recorded yet.', 'aiya-core'); ?></td></tr>
                <?php else : ?>
    ...
    public function usersColumnValue(mixed $value, string $column, int $userId): string
    {
        ...
        $tier = $this->membership->currentTier($userId);
        if ($tier === null) {
            return '&mdash;';
        }

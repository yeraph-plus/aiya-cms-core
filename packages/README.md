...
1. **第一批**（近零耦合，直接迁移）：`opencc-convert`（首个 tracer 包，已建；**零消费**——简繁重建待站长拍板，2026-09-19，非待办工作项）、`multi-domain`；`internal-pic-bed` **不做包**——已按功能直接做成 core 内页面（0.9.0 `src/Admin/PicBedPage.php`，0.9.1 起为主菜单项；图片处理管线以闭包注入，不引包依赖）
...
| 包 | 状态 |
|---|---|
| `opencc-convert` | 包体就绪（Converter + locale 策略映射），**零消费**——简繁重建待站长拍板（2026-09-19），非待办工作项；若拍板接线，在根 `composer.json` 补 `overtrue/php-opencc`（当前刻意不 require） |
...
| `payment-afdian` | ✅ 0.88.0 已接线（零三方依赖；0.92.0 起 **webhook 不验签**——RSA verify 与 `ext-openssl` 整体摘除，推送只可信订单号、站点经鉴权 open-API 查单结算）：`Client`（open-API md5 出站签名、transport 闭包）+ `Gateway`（订单深链、回调形态与归一化、`afd_` 前缀、周期钳制；0.93.0 起 `orderUrl()` 支持按档位传入 planId 覆盖）；依赖同仓包 `slug-toolkit`（包→包，`require` 不声明）；消费方为 `Domain/Sponsorship/AfdianGateway`（薄适配器）+ `AfdianActivator`（留在核心，它要写钱与权益） |
| `typesetting` | ✅ 0.37.0 已接线（零三方依赖，仅 `ext-json`）：`ChineseTypesetting`（jxlwqq/chinese-typesetting 原样移植）；消费方为 `Domain/Content/TypographyModule` |
| `openlist` | ✅ 0.90.0 重写为**纯请求出口**（零三方依赖）：`Client`（登录 + fs 读操作，transport 闭包注入，平台错误分级成包内 `Error`）+ `Gateway`（`list()` / `search()` 两个 surfacing 调用 → 纯数组行：name / kind / size / modified / path / url；链接本地拼接 f·d·p，不留 `raw_url` 的逐文件请求）；**不与 core 共享任何类型**；0.93.0 起 `Gateway` 构造增 `linkBase`——投放链接按浏览器可达基址拼、API 客户端继续走容器内网地址（设置 `fileserve_oplist_public_url`）；消费方为 `Modules/OpenListModule`（薄适配器：设置读取 / wp_remote transport / token 缓存）与 `Domain/FileServe/Adapters/OpenListAdapter`（映射成站内 `Entry`） |

...
| `inc/func-payment.php` (Afdian integration + redemption codes) | ✅ 0.24.0 core slice: Domain/Sponsorship — order store + stacking fold + `sponsor_expiration` writer (OrderService), atomic redemption with rollback (RedeemCodeService), domain settings page, plans/redeem/membership REST. Remaining (gateway slices, unbatched): Afdian webhook (adding signature verification) + order-number redemption; the legacy access settings page is not migrated |
| `plugins/sponsor-order-compat` (Epay gateway) | Domain/Sponsorship gateway slice (unbatched): cashier flow + signed callback + anti-cross-order check preserved; days resolution by price matching is replaced by the plan key carried in signed params |
